...
libraries <-c("COVID19", "tidyverse", "skimr", "prettydoc", "rnaturalearth", "sf","tmap" ,"sf", "maps","viridis","readr","ggplot2","rnaturalearth","ggpubr","kableExtra","wbstats")
...
```{r}
cov19 %>% filter(administrative_area_level==1,!is.na(confirmed),
...
```{r}
w = dplyr::left_join(world_projected, cov19,
                     by = c("ISO_A3_EH"= "iso_alpha_3"))
...
Si on essaye de voir une carte similaire mais avec le nombre de cas confirmés :
...
```
```{r}
plot(gdata_france["confirmed"],
     main="Nombre de cas confirmés en France par département", logz = TRUE)
...
```{r}
plot(gdata_france["population"],
     main="Nombre d'habitants en France par département", logz = TRUE)
```
...
ggscatter(cov19_corr, x = "population", y = "confirmed", 
          ...
          xlab = "La population par département",
          ylab = "Nombre de cas confirmés")
```

```{r,echo = FALSE}
knitr::include_graphics("population.png")
```



A l'échelle mondiale (il y a juste une relation positive modérée):
...
Dans cette partie, nous allons voir d'autres éléments et variables dans nos données.
###  L'espérance de vie 
Nous pouvons commencer par voir l'espérance de vie et sa relation avec le nombre total de décès pour chaque pays.

```{r, warning=FALSE}
#Pour obtenir l'espérance de vie par pays, 
#nous utiliserons les données de la Banque Mondiale
#et nous utiliserons directement cette fonction donnée
#pour extraire la base de données nécessaire.
wb_data_create <-  function(indicator, new_name, year, ...){
  df <-  wb(indicator = indicator,
          startdate = year, enddate = year, ...) %>%
    select(iso_a3 = iso3c, value, year = date) %>%
...
```{r}
cov19_corr <- select(cov19,deaths,date,iso_alpha_3)  %>%
  filter( !is.na(deaths))  %>%
  filter(date == max(date, na.rm = T) - 30)
...
```{r}
w_corr <-  dplyr::left_join(data_life_exp, cov19_corr, 
                            by = c("iso_a3"= "iso_alpha_3"))  %>%
  select(life_exp,deaths)  %>%
  filter( !is.na(life_exp),!is.na(deaths)) 

ggscatter(w_corr, x = "life_exp", y = "deaths", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Espérance de vie ", ylab = "Nombre de décès") 

```

La relation ici est faible, et pas si significative. Même si le nombre de décès de Covid 19 est lié à des facteurs socio-économiques, il n'est pas corrélé à l'espérance de vie, et nous ne pouvons en tirer aucune conclusion. Par exemple, un pays où l'espérance de vie est élevée peut avoir plus de personnes âgées, ce qui le rend plus vulnérable, et avoir plus de décès. 

Nous verrons plutôt le taux de mortalité pour une population pour 1000 personnes, qui correspond au nombre de décès divisé par la taille de la population multipliée par 1000.


```{r, warning=FALSE}
cov19_corr <- select(cov19,deaths,population,date,iso_alpha_3)  %>%
  filter( !is.na(deaths),!is.na(population))  %>%
  filter(date == max(date, na.rm = T) - 30)
cov19_corr[1] <- cov19_corr[1]/cov19_corr[2]*1000
```
...
ggscatter(w_corr, x = "life_exp", y = "deaths", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Espérance de vie ",
          ylab = "Taux de mortalité pour une population") 

```

Ici, nous pouvons voir que le résultat est significatif (p-value) et la corrélation est plus forte mais il s'agit toujours d'une corrélation modérée (=0.45).
...
cov19_corr <- filter(cov19_corr, !is.na(deaths),
                     !is.na(elderly_people_protection))
cov19_corr <- filter(cov19_corr,date == max(date, na.rm = T) - 30)
cov19_corr <- group_by(cov19_corr,elderly_people_protection)
cov19_corr$elderly_people_protection <- 
  as.character(cov19_corr$elderly_people_protection)

box_plot <- ggplot(cov19_corr, aes(x = elderly_people_protection, y = deaths, 
                                   color = elderly_people_protection))+  scale_y_continuous(limits = 
   quantile(cov19_corr$deaths, c(0.0, 0.8)))
...
cov19_corr <- filter(cov19_corr,date == max(date, na.rm = T) - 30)
cov19_corr["elderly_people_protection"] <- 
  lapply(cov19_corr["elderly_people_protection"],  abs)
cov19_corr <- group_by(cov19_corr,elderly_people_protection)
cov19_corr$elderly_people_protection <- 
  as.character(cov19_corr$elderly_people_protection)

box_plot <- ggplot(cov19_corr, aes(x = elderly_people_protection, y = deaths, fill=elderly_people_protection))+  
  scale_y_continuous(limits = quantile(cov19_corr$deaths, c(0.0, 0.85)))
...
box_plot <- ggplot(cov19_corr, aes(x = testing_policy, y = confirmed, 
                                   fill=testing_policy))+  
  scale_y_continuous(limits = quantile(cov19_corr$confirmed, c(0.0, 0.9)))
...
- La démographie de la population est l'un des facteurs importants liés à la propagation et aux décès liés au Covid-19.
